...
log_debug("[SYSTEM] Program Started")
...
class AseAI:
    def __init__(self, master):
        self.master = master; self.spawn_x = random.randint(300, 1500); self.x, self.y = self.spawn_x, 500
    ...
    def update(self, ground_y):
        ...
        if self.ai_timer <= 0:
            if abs(dist_p) < 600:
                choices = ["IDLE", "CHASE", "ATTACK", "DASH", "JUMP"]
                self.decision = random.choice(choices)
            else:
                self.decision = random.choice(["IDLE", "WALK_L", "WALK_R"])
            self.ai_timer = random.randint(40, 120)
            if self.decision == "ATTACK" and abs(dist_p) < 200:
                self.facing_right = dist_p > 0; self.trigger_action(f"ComboAttack_{random.randint(1,4)}")
            elif self.decision == "DASH":
                self.facing_right = dist_p > 0; self.trigger_action("DASH")
            elif self.decision == "JUMP" and self.grounded:
                self.vy = self.master.jump_power; self.grounded = False
        ...
        if not self.active_tag_info:
            ...
            elif self.decision == "CHASE":
                self.vx = 5.5 if dist_p > 0 else -5.5; self.facing_right = dist_p > 0
...
class AsepritePlayer:
    def __init__(self, initial_path):
        ...
        self.bg_off_x = 0; self.bg_off_y = 0; self.bg_alpha = 255; self.bg_parallax = 0.1
        # Status
    ...
    def auto_map_source(self, source):
        ...
        for slot in self.mappings.keys():
            ...
            def sort_key(item):
                tl = item[1].lower(); return 0 if "ready" in tl else (2 if "end" in tl else 1)
    ...
    def trigger_action(self, slot, keys=None):
        ...
        if slot == "DASH" and self.dash_charges > 0:
            ...
            self.dash_timer = 12; self.vx = self.dash_speed if self.facing_right else -self.dash_speed; self.vy = 0; self.active_action_slot = "DASH"; self.action_queue = list(tags); self.play_next_in_queue(); self.loop_counter = 0; return
        if tags:
    ...
    def play_next_in_queue(self):
        if self.action_queue:
            ...
            if self.active_tag_info[1] in src.tags: self.frame_idx, self.action_end_frame = src.tags[self.active_tag_info[1]]; self.loop_counter = 0; log_debug(f"[ACTION] Started: {self.active_tag_info[1]}")
            else: self.play_next_in_queue()
        else: self.active_tag_info = None; self.active_action_slot = None; self.loop_counter = 0
    def update(self, keys, ground_y, play_w, play_h):
        ...
        # Ground hit: Break falling loops for specific actions
        if self.grounded and self.active_action_slot in ["POWERBOMB", "JUMPATTACK"]:
            if self.active_tag_info and "(loop)" in self.active_tag_info[1].lower():
                log_debug(f"[ACTION] Ground Hit! Breaking loop: {self.active_tag_info[1]}")
                self.play_next_in_queue()
        ...
        if target_info:
            ...
            if self.anim_timer > 6:
                ...
                if self.active_tag_info and self.frame_idx > self.action_end_frame:
                    ...
                    if is_loop and is_skill and self.loop_counter < 1: 
                        log_debug(f"[LOOP] Skill {target_info[1]} repeat {self.loop_counter+1}/2")
                        self.frame_idx = tr[0]; self.loop_counter += 1
    ...
    def draw(self, screen, play_w, play_h):
        ...
        for ai in self.ai_list:
            ai_s = ai.active_tag_info[0] if ai.active_tag_info else 0; self.draw_sprite(screen, ai.x, ai.y, ai_s, ai.frame_idx, ai.facing_right, self.cam_x, self.cam_y, cx, cy)
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if player:
                    for i in range(len(player.sources)):
                        if pygame.Rect(190+i*110, 5, 100, 30).collidepoint(m_pos): player.current_source_tab = i
                    if play_w < m_pos[0] < sw:
                        if not show_settings:
                            ...
                            if event.button == 1:
                                ...
                                if selected_slot:
                                    src = player.sources[player.current_source_tab]
                            ...
                            for cat in folds.keys():
                                if pygame.Rect(play_w+10, fy, sidebar_w-20, 30).collidepoint(m_pos): folds[cat] = not folds[cat]
                                fy += 35
                                if folds[cat]:
                                    if cat=="PHYSICS": fy += 140
                                    elif cat=="AI & COMBAT": fy += 100
                                    elif cat=="BG IMAGE": fy += 200
                                    elif cat=="BG COLOR": fy += 180
        ...
        if player:
            if show_settings:
                    ...
                    if folds[cat]:
                        if cat == "PHYSICS":
                            for i, (l, mn, mx, at, inv) in enumerate([("Dash Vel",10,50,"dash_speed",0), ("Jump Pow",10,25,"jump_power",1), ("PBomb Spd",10,60,"powerbomb_speed",0)]):
                                y = cy+i*45; screen.blit(font_s.render(l, True, (150,150,150)), (play_w+20, y)); sl = pygame.Rect(play_w+20, y+18, sidebar_w-40, 8); pygame.draw.rect(screen, (60,60,70), sl); v = getattr(player, at);
                                n = (v-mn)/(mx-mn) if not inv else (-v-mn)/(mx-mn); pygame.draw.circle(screen, (59,130,246), (int(play_w+20+n*(sidebar_w-40)), y+22), 8)
                                if pygame.mouse.get_pressed()[0] and sl.inflate(0,20).collidepoint(m_pos): 
                                    nv = mn+(m_pos[0]-(play_w+20))/(sidebar_w-40)*(mx-mn); setattr(player, at, nv if not inv else -nv)
                            cy += 140
                        elif cat == "AI & COMBAT":
                            ...
                            if pygame.mouse.get_pressed()[0] and bg_btn.collidepoint(m_pos):
                                ...
                                if p: player.bg_img = pygame.image.load(p).convert_alpha(); log_debug(f"[BG] Image loaded: {p}")
                            cy += 40
                ...
                for i, action in enumerate(player.mappings.keys()):
                    ...
                    ms = ", ".join([f"{player.sources[m[0]].name[:3]}:{m[1]}" for m in player.mappings[action]]); slot_clip.blit(font_s.render(f"-> {ms[:45]}", True, (200,200,200) if not is_sel else (255,255,255)), (rect.x+10, rect.y+18))
            ...
            pygame.draw.rect(screen, (30,30,35), (0, sh-40, play_w, 40)); guide = "Arrows: Walk | Space: Jump | Atk: Z | PBomb: ↓+Z | Dash: X | Skill: C,B,N | Wheel: Zoom"; screen.blit(font_s.render(guide, True, (255,255,255)), (20, sh-25))
